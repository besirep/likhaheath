...
const statusStyle = {
  ...
  urgent:   { color: "#e07050", bg: "#fde8e0", label: "Urgent"   },
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "white", border: "1px solid #dce5f3", borderRadius: 10, padding: "8px 12px", boxShadow: "0 4px 16px rgba(20,40,90,0.1)", fontSize: 12 }}>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <div style={{ fontSize: 19, fontWeight: 700, color: "white", fontFamily: "'Fraunces', serif" }}>{visit?.diagnosis}</div>
        ...
        <div style={{ flex: 1, padding: "22px 24px", display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          {/* Vitals */}
          <div>
            <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 10 }}>Vitals at This Visit</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
              {v && [
                ...
                <div key={f.label} style={{
                  ...
                  <div style={{ fontSize: 17, fontWeight: 700, color: flagColor[f.flag], fontFamily: "'Fraunces', serif", lineHeight: 1 }}>{f.value}</div>
                  <div style={{ fontSize: 9, color: "#9aabc0", marginTop: 1 }}>{f.unit}</div>
          ...
          <div style={{ background: "#f7f9fd", borderRadius: 13, padding: "16px 18px", border: "1px solid #e8edf7" }}>
            <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 8 }}>📝 Doctor's Notes</div>
            <div style={{ fontSize: 13, color: "#2a3550", lineHeight: 1.75 }}>{visit?.notes}</div>
          </div>

          {/* Treatment plan */}
          <div style={{ background: "#eef3fc", borderRadius: 13, padding: "16px 18px", border: "1px solid #d0ddf5" }}>
            <div style={{ fontSize: 11, color: "#3b7dd8", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 8 }}>📋 Treatment Plan</div>
            <div style={{ fontSize: 13, color: "#2a3550", lineHeight: 1.75 }}>{visit?.plan}</div>
          </div>

          {/* Labs + follow-up */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ background: "white", borderRadius: 13, padding: "14px 16px", border: "1px solid #e0e7f3" }}>
              <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 10 }}>🧪 Labs Ordered</div>
              {visit?.labs?.length > 0 ? (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {visit.labs.map(l => (
                    <span key={l} style={{ background: "#eef3fc", color: "#3b7dd8", borderRadius: 7, padding: "3px 10px", fontSize: 12, fontWeight: 500 }}>{l}</span>
                  ))}
                </div>
              ) : (
                <div style={{ fontSize: 13, color: "#9aabc0" }}>None ordered</div>
              )}
            </div>
            <div style={{ background: "white", borderRadius: 13, padding: "14px 16px", border: "1px solid #e0e7f3" }}>
              <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 10 }}>📅 Follow-up</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>{visit?.followUp || "—"}</div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            <button style={{
              flex: 1, background: "#f0f4fb", border: "1px solid #dce5f3", borderRadius: 10,
              padding: "10px", fontSize: 13, color: "#3b7dd8", cursor: "pointer", fontWeight: 600,
            }}>🖨 Print Record</button>
            <button style={{
              flex: 1, background: "#1a2540", border: "none", borderRadius: 10,
              padding: "10px", fontSize: 13, color: "white", cursor: "pointer", fontWeight: 600,
            }}>✏️ Edit Notes</button>
          </div>
        </div>
      </div>
...
function ProfilePanel({ patient, onVisitSelect, selectedVisitId }) {
  ...
  if (!patient) return (
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, color: "#9aabc0" }}>
      ...
      <div style={{ fontSize: 13 }}>Click any record to view their profile</div>
    </div>
  );
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#1a2540 0%,#263760 100%)", padding: "28px 28px 22px", flexShrink: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              <div style={{ fontSize: 22, fontWeight: 700, color: "white", fontFamily: "'Fraunces', serif" }}>{patient.name}</div>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginTop: 3 }}>{patient.age} yrs · {patient.gender} · DOB: {patient.dob}</div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button style={{ background: "rgba(255,255,255,0.1)", color: "white", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 10, padding: "8px 16px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>📅 Schedule</button>
            <button style={{ background: "white", color: "#1a2540", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>✏️ Edit</button>
          </div>
        </div>
        <div style={{ display: "flex", gap: 20, marginTop: 16, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          {[{ icon: "📞", label: patient.contact }, { icon: "📍", label: patient.address }, { icon: "📅", label: `Last visit: ${patient.lastVisit}` }].map(item => (
            <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "rgba(255,255,255,0.6)" }}>
      ...
      {/* Tabs */}
      <div style={{ background: "white", borderBottom: "1px solid #dce5f3", padding: "0 28px", display: "flex", flexShrink: 0 }}>
        {["overview", "vitals", "history"].map(t => (
          <button key={t} onClick={() => setActiveTab(t)} style={{
            ...
            color: activeTab === t ? "#3b7dd8" : "#7a8fb0",
            borderBottom: activeTab === t ? "2.5px solid #3b7dd8" : "2.5px solid transparent",
      ...
      <div style={{ flex: 1, padding: "24px 28px", background: "#f7f9fd" }}>
        ...
        {activeTab === "overview" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div style={{ background: "white", borderRadius: 14, padding: "18px 20px", border: "1px solid #e0e7f3" }}>
                <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 12 }}>🏥 Active Conditions</div>
                {patient.conditions.length > 0 ? patient.conditions.map(c => (
                  <div key={c} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#3b7dd8", flexShrink: 0 }} />
                    <span style={{ fontSize: 14, color: "#1a2540", fontWeight: 500 }}>{c}</span>
                  </div>
                )) : <div style={{ fontSize: 13, color: "#9aabc0" }}>No active conditions</div>}
              </div>
              <div style={{ background: "white", borderRadius: 14, padding: "18px 20px", border: "1px solid #e0e7f3" }}>
                <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 12 }}>⚠️ Allergies</div>
                {patient.allergies.length > 0 ? (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                    {patient.allergies.map(a => (
                      <span key={a} style={{ background: "#fde8e0", color: "#e07050", borderRadius: 8, padding: "5px 12px", fontSize: 13, fontWeight: 600, border: "1px solid #f5c8b0" }}>⚠ {a}</span>
                    ))}
                  </div>
                ) : <div style={{ fontSize: 13, color: "#9aabc0" }}>No known allergies</div>}
              </div>
            </div>
            <div style={{ background: "white", borderRadius: 14, padding: "18px 20px", border: "1px solid #e0e7f3" }}>
              <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 14 }}>👤 Personal Information</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 0 }}>
                {[
                  ...
                  <div key={f.label} style={{ padding: "12px 0", borderBottom: i < 3 ? "1px solid #f0f3fa" : "none", paddingRight: 16 }}>
                    <div style={{ fontSize: 10, color: "#9aabc0", marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.4 }}>{f.label}</div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#1a2540" }}>{f.value}</div>
                  </div>
                ))}
              </div>
            </div>
            {patient.visits[0] && (
              <div style={{ background: "linear-gradient(135deg,#eef3fc,#e4ecfb)", borderRadius: 14, padding: "18px 20px", border: "1px solid #c0d4f5", cursor: "pointer" }}
                onClick={() => { onVisitSelect(patient.visits[0]); setActiveTab("history"); }}>
                <div style={{ fontSize: 11, color: "#3b7dd8", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 8 }}>🩺 Last Consultation — click to view</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>{patient.visits[0].diagnosis}</div>
                <div style={{ fontSize: 13, color: "#5a6f90", marginTop: 3 }}>{patient.visits[0].date} · {patient.visits[0].reason}</div>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === "vitals" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "white", borderRadius: 14, padding: "20px 22px", border: "1px solid #e0e7f3" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>Blood Pressure Trend</div>
                  <div style={{ fontSize: 12, color: "#9aabc0", marginTop: 2 }}>Systolic over time (mmHg)</div>
                </div>
              </div>
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={patient.vitalsHistory}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f3fa" vertical={false} />
                  <XAxis dataKey="date" tick={{ fontSize: 11, fill: "#9aabc0" }} axisLine={false} tickLine={false} />
                  <YAxis domain={[80, 180]} tick={{ fontSize: 11, fill: "#9aabc0" }} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="bp" name="BP" stroke="#e07050" strokeWidth={2.5} dot={{ r: 4, fill: "#e07050" }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {[
                { key: "hr",     name: "HR",     label: "Heart Rate", unit: "bpm",  color: "#3b7dd8", domain: [60, 110] },
                { key: "weight", name: "Weight", label: "Weight",     unit: "kg",   color: "#2a9d8f", domain: [40, 100] },
              ].map(chart => (
                <div key={chart.key} style={{ background: "white", borderRadius: 14, padding: "18px 20px", border: "1px solid #e0e7f3" }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif", marginBottom: 2 }}>{chart.label}</div>
                  <div style={{ fontSize: 12, color: "#9aabc0", marginBottom: 14 }}>{chart.unit} over time</div>
                  <ResponsiveContainer width="100%" height={130}>
                    <LineChart data={patient.vitalsHistory}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f3fa" vertical={false} />
                      <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#9aabc0" }} axisLine={false} tickLine={false} />
                      <YAxis domain={chart.domain} tick={{ fontSize: 10, fill: "#9aabc0" }} axisLine={false} tickLine={false} />
                      <Tooltip content={<CustomTooltip />} />
        ...
        {activeTab === "history" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            ...
            <div style={{ position: "relative" }}>
              {/* Vertical line */}
              <div style={{ position: "absolute", left: 16, top: 10, bottom: 10, width: 2, background: "#e0e7f3", zIndex: 0 }} />
                ...
                return (
                  <div key={v.id} onClick={() => onVisitSelect(v)} style={{
                    ...
                    <div style={{
                      ...
                      background: i === 0 ? "#3b7dd8" : "white",
                      border: `2px solid ${i === 0 ? "#3b7dd8" : "#d0ddf5"}`,
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 14, boxShadow: i === 0 ? "0 0 0 4px #ddeafc" : "none",
                      zIndex: 2,
                    }}>
                      {i === 0 ? <span style={{ fontSize: 14 }}>🩺</span> : <span style={{ fontSize: 11, fontWeight: 700, color: "#9aabc0" }}>{patient.visits.length - i}</span>}
                    </div>

                    {/* Card */}
                    <div style={{
                      flex: 1, background: "white", borderRadius: 14, padding: "16px 18px",
                      border: `2px solid ${isSelected ? "#3b7dd8" : "#e0e7f3"}`,
                      boxShadow: isSelected ? "0 4px 20px rgba(59,125,216,0.14)" : "0 2px 8px rgba(20,40,90,0.04)",
                      transition: "all 0.18s",
                    }}
                      onMouseEnter={e => { if (!isSelected) { e.currentTarget.style.borderColor = "#b0c8f0"; e.currentTarget.style.boxShadow = "0 4px 16px rgba(59,125,216,0.1)"; }}}
                      onMouseLeave={e => { if (!isSelected) { e.currentTarget.style.borderColor = "#e0e7f3"; e.currentTarget.style.boxShadow = "0 2px 8px rgba(20,40,90,0.04)"; }}}
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>{v.diagnosis}</div>
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 12 }}>
                          <div style={{ fontSize: 12, fontWeight: 600, color: i === 0 ? "#3b7dd8" : "#7a8fb0" }}>{v.date}</div>
                          <div style={{ fontSize: 11, color: "#b0bdd6", marginTop: 2 }}>{v.queue} · {v.duration}</div>
                          <div style={{
                            marginTop: 8, background: isSelected ? "#3b7dd8" : "#f0f4fb",
                            color: isSelected ? "white" : "#3b7dd8",
                            border: `1px solid ${isSelected ? "#3b7dd8" : "#dce5f3"}`,
                            borderRadius: 7, padding: "4px 12px", fontSize: 11, fontWeight: 600, display: "inline-block",
...
export default function DoctorPatientRecords() {
  ...
  const handlePatientSelect = (p) => {
    setSelected(p);
    setActiveVisit(null); // close drawer when switching patients
  };
    ...
    <div style={{ height: "100vh", background: "#f0f4fb", fontFamily: "'DM Sans',sans-serif", display: "flex", overflow: "hidden" }}>
      ...
      <div style={{ marginLeft: 220, flex: 1, display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ background: "#f0f4fb", borderBottom: "1px solid #dce5f3", padding: "14px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontFamily: "'Fraunces',serif", fontWeight: 700, color: "#1a2540" }}>Patient Records</h1>
            <div style={{ fontSize: 13, color: "#7a8fb0", marginTop: 2 }}>{patients.length} patients · {patients.reduce((a, p) => a + p.visits.length, 0)} total consultation records</div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <div style={{ display: "flex", gap: 7 }}>
              {[
                { label: "Patients",    value: patients.length,                                       color: "#3b7dd8", bg: "#eef3fc" },
                { label: "Active",      value: patients.filter(p => p.status === "active").length,    color: "#2a7d5f", bg: "#e8f7f1" },
                { label: "Consultations", value: patients.reduce((a, p) => a + p.visits.length, 0),  color: "#7b5ea7", bg: "#f0eafb" },
              ].map(s => (
            ...
            <button style={{ background: "linear-gradient(135deg,#3b7dd8,#5b9af0)", color: "white", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", boxShadow: "0 4px 14px rgba(59,125,216,0.3)", display: "flex", alignItems: "center", gap: 6 }}>➕ Add Patient</button>
          </div>
        </div>
              ...
              <div style={{ display: "flex", gap: 5 }}>
                {["all", "active", "urgent", "inactive"].map(s => (
                  <button key={s} onClick={() => setStatusFilter(s)} style={{
                    background: statusFilter === s ? "#1a2540" : "#f0f4fb",
              ...
              {filtered.map(p => {
                ...
                return (
                  <div key={p.id} onClick={() => handlePatientSelect(p)} style={{
                    ...
                    border: `1.5px solid ${isSelected ? "#c0d4f5" : "transparent"}`,
                    transition: "all 0.15s", position: "relative", overflow: "hidden",
                  }}
                    onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = "#f7f9fd"; }}
                    onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = "transparent"; }}
                  >
                    {p.status === "urgent" && <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 3, background: "#e07050", borderRadius: "12px 0 0 12px" }} />}
                    <div style={{ display: "flex", gap: 10, paddingLeft: p.status === "urgent" ? 6 : 0 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          ...
                          <span style={{ background: ss.bg, color: ss.color, borderRadius: 5, padding: "1px 7px", fontSize: 10, fontWeight: 600, flexShrink: 0, marginLeft: 4 }}>{ss.label}</span>
                        </div>
                        <div style={{ fontSize: 12, color: "#7a8fb0", marginTop: 1 }}>{p.age} yrs · {p.gender} · {p.bloodType}</div>
                        <div style={{ fontSize: 11, color: "#9aabc0", marginTop: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {p.conditions.length > 0 ? p.conditions.join(" · ") : "No active conditions"}
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                          <div style={{ fontSize: 11, color: "#b0bdd6" }}>Last visit: {p.lastVisit}</div>
                          <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 500 }}>{p.visits.length} records</div>
                        </div>
                      </div>
                    </div>
                  </div>
