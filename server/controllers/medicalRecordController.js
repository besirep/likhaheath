...
exports.create = async (req, res) => {
  ...
  // RBAC: Doctors can only create their own records
  if (req.user.role === 'Doctor' && req.user.doctorId != doctor_id)
  ...
  try {
    const [result] = await db.query(
      `INSERT INTO medical_records (patient_id, appointment_id, doctor_id, diagnosis, treatment, notes, record_date)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [patient_id, appointment_id || null, doctor_id || req.user.doctorId || null, diagnosis, treatment, notes || null, record_date]
    );
