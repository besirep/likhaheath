...
const typeConfig = {
  "follow-up": { label: "Follow-up", color: "#3b7dd8", bg: "#eef3fc", dot: "#3b7dd8" },
...
const statusConfig = {
  scheduled: { label: "Scheduled", color: "#3b7dd8", bg: "#eef3fc" },
      ...
      <div style={{
        ...
        <div style={{ background: "linear-gradient(135deg,#1a2540,#263760)", padding: "24px", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 4 }}>Appointment Detail</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "white", fontFamily: "'Fraunces', serif" }}>{appt.name}</div>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginTop: 3 }}>{appt.age} yrs · {appt.reason}</div>
            </div>
        ...
        <div style={{ flex: 1, padding: "22px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          {/* Time block */}
          <div style={{ background: "#eef3fc", borderRadius: 14, padding: "16px 18px", border: "1px solid #d0ddf5" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 11, color: "#7a8fb0", marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>Date & Time</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>
                  {new Date(appt.date).toLocaleDateString("en-PH", { weekday: "long", month: "long", day: "numeric" })}
                </div>
                <div style={{ fontSize: 15, color: "#3b7dd8", fontWeight: 600, marginTop: 4 }}>
                  {appt.time} – {appt.end}
          ...
          {/* Patient info */}
          <div style={{ background: "white", borderRadius: 14, padding: "16px 18px", border: "1px solid #e0e7f3" }}>
            <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 12 }}>Patient Info</div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
              <Avatar name={appt.name} size={44} />
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>{appt.name}</div>
                <div style={{ fontSize: 12, color: "#7a8fb0" }}>{appt.age} years old</div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#4a5d75" }}>
          ...
          {/* Notes */}
          {appt.notes && (
            <div style={{ background: "#f7f9fd", borderRadius: 14, padding: "16px 18px", border: "1px solid #e0e7f3" }}>
              <div style={{ fontSize: 11, color: "#9aabc0", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.7, marginBottom: 8 }}>📝 Notes</div>
              <div style={{ fontSize: 13, color: "#2a3550", lineHeight: 1.7 }}>{appt.notes}</div>
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: "auto" }}>
            {appt.status === "scheduled" && (
              <>
                <button style={{
                  background: "linear-gradient(135deg,#3b7dd8,#5b9af0)", color: "white", border: "none",
                  borderRadius: 11, padding: "13px", fontSize: 14, fontWeight: 700, cursor: "pointer",
                  boxShadow: "0 4px 16px rgba(59,125,216,0.3)",
                }}>🩺 Start Consultation</button>
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={{
                    flex: 1, background: "#f0f4fb", color: "#3b7dd8", border: "1px solid #dce5f3",
                    borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 600, cursor: "pointer",
                  }}>✏️ Reschedule</button>
                  <button onClick={() => onCancel(appt)} style={{
                    flex: 1, background: "#fff0ee", color: "#e07050", border: "1px solid #f5c8b0",
                    borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 600, cursor: "pointer",
                  }}>✕ Cancel</button>
                </div>
                <button style={{
                  background: "white", color: "#7a8fb0", border: "1px solid #dce5f3",
                  borderRadius: 10, padding: "10px", fontSize: 13, cursor: "pointer",
                }}>📱 Send Reminder SMS</button>
              </>
            )}
            {appt.status === "completed" && (
              <div style={{ background: "#e8f7f5", borderRadius: 11, padding: "13px", textAlign: "center", color: "#2a9d8f", fontSize: 13, fontWeight: 600 }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "24px 26px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 80px", gap: 10 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: "#9aabc0", textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>Patient Name</label>
              <input value={form.name} onChange={e => update("name", e.target.value)} placeholder="Full name"
                style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #e0e7f3", borderRadius: 10, fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: "#1a2540", outline: "none", boxSizing: "border-box" }}
                onFocus={e => e.target.style.borderColor = "#3b7dd8"} onBlur={e => e.target.style.borderColor = "#e0e7f3"} />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: "#9aabc0", textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>Age</label>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9aabc0", textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>Contact Number</label>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9aabc0", textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>Reason for Visit</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {[
              ...
              <div key={f.key}>
                <label style={{ fontSize: 11, fontWeight: 600, color: "#9aabc0", textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>{f.label}</label>
          ...
          {/* Type */}
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9aabc0", textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 8 }}>Appointment Type</label>
            <div style={{ display: "flex", gap: 8 }}>
              {Object.entries(typeConfig).map(([key, tc]) => (
                <button key={key} onClick={() => update("type", key)} style={{
                  flex: 1, padding: "8px 6px", border: `2px solid ${form.type === key ? tc.color : "#e0e7f3"}`,
                  borderRadius: 10, background: form.type === key ? tc.bg : "white",
                  color: form.type === key ? tc.color : "#7a8fb0",
                  fontSize: 11, fontWeight: form.type === key ? 700 : 400,
          ...
          {/* Notes */}
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9aabc0", textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>Notes (optional)</label>
            <textarea value={form.notes} onChange={e => update("notes", e.target.value)} placeholder="Any prep instructions or reminders for the patient..."
              rows={3} style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #e0e7f3", borderRadius: 10, fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: "#1a2540", outline: "none", resize: "none", boxSizing: "border-box" }}
              onFocus={e => e.target.style.borderColor = "#3b7dd8"} onBlur={e => e.target.style.borderColor = "#e0e7f3"} />
          </div>

          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button onClick={onClose} style={{ flex: 1, background: "#f0f4fb", border: "1px solid #dce5f3", borderRadius: 11, padding: "12px", fontSize: 13, color: "#7a8fb0", cursor: "pointer", fontWeight: 500 }}>Cancel</button>
            <button style={{ flex: 2, background: "linear-gradient(135deg,#3b7dd8,#5b9af0)", color: "white", border: "none", borderRadius: 11, padding: "12px", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 16px rgba(59,125,216,0.3)" }}>
              ✓ Book Appointment
            </button>
          </div>
        </div>
...
function MiniCalendar({ selectedDate, onSelect, appointments }) {
  ...
  return (
    <div style={{ background: "white", borderRadius: 16, padding: "18px", border: "1px solid #e0e7f3", boxShadow: "0 2px 10px rgba(20,40,90,0.06)" }}>
      {/* Nav */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 13, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>{monthName}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", marginBottom: 6 }}>
        {["S","M","T","W","T","F","S"].map((d, i) => (
          <div key={i} style={{ textAlign: "center", fontSize: 10, fontWeight: 600, color: "#b0bdd6", padding: "2px 0" }}>{d}</div>
        ))}
      </div>
        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          return (
            <div key={day} onClick={() => onSelect(dateStr)} style={{
              ...
              background: isSelected ? "#3b7dd8" : isToday ? "#eef3fc" : "transparent",
              color: isSelected ? "white" : isToday ? "#3b7dd8" : "#1a2540",
              fontWeight: isSelected || isToday ? 700 : 400,
              fontSize: 12, position: "relative", transition: "background 0.15s",
            }}
              onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = "#f0f4fb"; }}
              onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = isToday ? "#eef3fc" : "transparent"; }}
            >
              {day}
              {hasAppt && (
                <div style={{
                  width: 4, height: 4, borderRadius: "50%", margin: "1px auto 0",
                  background: isSelected ? "rgba(255,255,255,0.8)" : isUrgent ? "#e07050" : "#3b7dd8",
                }} />
              )}
            </div>
          );
...
function DaySchedule({ date, appointments, onSelect, selectedId }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* Day header */}
      <div style={{ padding: "16px 22px", borderBottom: "1px solid #e0e7f3", background: "white", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif" }}>{dateLabel}</div>
          <div style={{ fontSize: 12, color: "#7a8fb0", marginTop: 2 }}>{dayAppts.length} appointment{dayAppts.length !== 1 ? "s" : ""} scheduled</div>
        </div>
        {dayAppts.filter(a => a.type === "urgent").length > 0 && (
          <div style={{ background: "#fde8e0", color: "#e07050", borderRadius: 9, padding: "5px 12px", fontSize: 12, fontWeight: 700 }}>
            🚨 {dayAppts.filter(a => a.type === "urgent").length} urgent
          </div>
        )}
      </div>
                ...
                <div key={appt.id} onClick={() => onSelect(appt)} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: isSelected ? "white" : tc.color, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {appt.name}
                      </div>
                      {height > 45 && (
                        <div style={{ fontSize: 11, color: isSelected ? "rgba(255,255,255,0.8)" : "#7a8fb0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {appt.time}–{appt.end} · {appt.reason}
                        </div>
                      )}
                    </div>
                    <div style={{ marginLeft: "auto", fontSize: 10, fontWeight: 600, color: isSelected ? "rgba(255,255,255,0.8)" : tc.color, flexShrink: 0 }}>
                      {appt.duration}m
                    </div>
                  </div>
                </div>
      ...
      {upcoming.map(appt => {
        ...
        return (
          <div key={appt.id} onClick={() => onSelect(appt)} style={{
            ...
            border: `1.5px solid ${isSelected ? tc.color : "#e0e7f3"}`,
            cursor: "pointer", transition: "all 0.15s",
            boxShadow: isSelected ? `0 3px 12px ${tc.color}25` : "none",
          }}
            onMouseEnter={e => { if (!isSelected) e.currentTarget.style.borderColor = "#b0c8f0"; }}
            onMouseLeave={e => { if (!isSelected) e.currentTarget.style.borderColor = "#e0e7f3"; }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <Avatar name={appt.name} size={34} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#1a2540" }}>{appt.name}</div>
                  <span style={{ background: tc.bg, color: tc.color, borderRadius: 5, padding: "1px 7px", fontSize: 10, fontWeight: 600, flexShrink: 0, marginLeft: 6 }}>{tc.label}</span>
                </div>
                <div style={{ fontSize: 11, color: "#7a8fb0", marginTop: 2 }}>{appt.reason}</div>
                <div style={{ fontSize: 11, color: "#b0bdd6", marginTop: 3 }}>
    ...
    <div style={{ height: "100vh", background: "#f0f4fb", fontFamily: "'DM Sans',sans-serif", display: "flex", overflow: "hidden" }}>
      ...
      <div style={{ marginLeft: 220, flex: 1, display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ background: "#f0f4fb", borderBottom: "1px solid #dce5f3", padding: "14px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontFamily: "'Fraunces',serif", fontWeight: 700, color: "#1a2540" }}>Appointments</h1>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 7 }}>
              {[
                { label: "Today",     value: today.length,                                                color: "#3b7dd8", bg: "#eef3fc" },
                ...
                { label: "Urgent",    value: appointments.filter(a => a.type === "urgent").length,        color: "#e07050", bg: "#fde8e0" },
              ].map(s => (
            ...
            {/* View toggle */}
            <div style={{ display: "flex", background: "white", border: "1px solid #e0e7f3", borderRadius: 10, overflow: "hidden" }}>
            ...
            <button onClick={() => setShowBook(true)} style={{ background: "linear-gradient(135deg,#3b7dd8,#5b9af0)", color: "white", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", boxShadow: "0 4px 14px rgba(59,125,216,0.3)", display: "flex", alignItems: "center", gap: 6 }}>
              ➕ Book Appointment
            </button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: "220px 1fr", overflow: "hidden" }}>
          ...
          <div style={{ background: "white", borderRight: "1px solid #e0e7f3", padding: "16px 14px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 16 }}>
            <MiniCalendar
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                { label: "This Week",  value: appointments.filter(a => a.date >= "2026-03-01" && a.date <= "2026-03-07").length, color: "#3b7dd8", bg: "#eef3fc" },
                { label: "Urgent",     value: appointments.filter(a => a.type === "urgent").length,                               color: "#e07050", bg: "#fde8e0" },
            ...
            <div style={{ background: "#f7f9fd", borderRadius: 12, padding: "12px" }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: "#b0bdd6", textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 8 }}>Legend</div>
              {Object.entries(typeConfig).map(([key, tc]) => (
          ...
          <div style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {view === "day" ? (
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: "#1a2540", fontFamily: "'Fraunces', serif", marginBottom: 16 }}>
