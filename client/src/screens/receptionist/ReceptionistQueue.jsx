        ...
        <div style={{ flex: 1, padding: "20px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: "auto" }}>
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => onAction("dismiss", patient)} style={{ flex: 1, background: "white", color: "#c05080", border: "1px solid #f0c0d8", borderRadius: 10, padding: "9px", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><X size={14} /> Delete</button>
              </div>
            </>
          )}
          {patient.status === "vitals-done" && (
            ...
            {patient.status === "skipped" && (
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => onAction("requeue", patient)} style={{ flex: 2, background: "linear-gradient(135deg,#2a9d8f,#52c4b8)", color: "white", border: "none", borderRadius: 11, padding: "13px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
                  ↺ Re-queue Patient
                </button>
                <button onClick={() => onAction("dismiss", patient)} style={{ flex: 1, background: "#fce8f0", color: "#c05080", border: "1px solid #f0c0d8", borderRadius: 11, padding: "13px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
                  <X size={14} /> Delete
                </button>
              </div>
            )}
...
export default function ReceptionistQueue({ onNavigate }) {
  ...
  const handleAction = async (action, patient) => {
    ...
    if (action === "dismiss") {
      ...
      setSelected(null);
      showToast(`${patient.name} removed from queue`);
      try {
        await queueApi.remove(patient.queueDbId);
      } catch (err) {
        showToast(`Failed to remove — please try again`);
        setQueue(q => [...q, patient]); // revert
      }
      return;
    }
